...
class Controleacesso extends CI_Model{
    ...
    if($title != FALSE) {
      ...
      return $query->row_array();
        ...
        function menus(){
        $query = $this->db->query("SELECT * FROM menu INNER JOIN acesso_menu ON menu.id_menu=acesso_menu.menu_id ORDER BY nome_menu");
              foreach ($query->result() as $row){ 
...
<li > <a data-toggle="dropdown" data-target="#<?php //echo $row->apelido;?>" href="#<?php //echo $row->link_menu;?>"><?php echo $row->nome_menu;?>   <span class="caret"></span></a>
                  ...
                  foreach ($querysub->result() as $rowsub){  
                     ...
                     <li class='has-sub'> <a href="<?php echo $rowsub->link_menu;?>"><?php echo $rowsub->nome_menu;?> </a></li>
