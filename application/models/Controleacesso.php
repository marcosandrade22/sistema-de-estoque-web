      ...
      return $query->row_array();
        ...
        $query = $this->db->query("SELECT * FROM menu INNER JOIN acesso_menu ON menu.id_menu=acesso_menu.menu_id ORDER BY ordem_menu");
              foreach ($query->result() as $row){
               ...
               <!-- inicio menu -->
               <li>
                 <a data-toggle="dropdown" data-target="#<?php //echo $row->apelido;?>" href="#<?php //echo $row->link_menu;?>">
                   <?php echo $row->nome_menu;?>   <span class="caret"></span>
                 </a>
                  <?php
        ...
        $query = $this->db->query("SELECT * FROM menu INNER JOIN acesso_menu ON menu.id_menu=acesso_menu.menu_id ORDER BY ordem_menu");
              foreach ($query->result() as $row){
                    if($row->pai_menu == 0 AND $row->funcao_id == $this->session->userdata("Funcao")){
                      if($this->uri->segment(1) == $row->link_menu){
               ...
               ?>
               <!-- inicio menu -->
               <a  href="<?php echo $row->link_menu;?>">
                 <i class="<?php echo $row->icone_menu;?>"></i>
                   <p><?php echo $row->nome_menu;?> </p>
                </a>
               </li>
                  <?php
                  }
              ...
              function menu_dash(){
                ...
                $query = $this->db->query("SELECT * FROM menu INNER JOIN acesso_menu ON menu.id_menu=acesso_menu.menu_id  WHERE link_menu='$link' ORDER BY ordem_menu");
                      foreach ($query->result() as $row){
                            if($row->pai_menu == 0 AND $row->funcao_id == $this->session->userdata("Funcao")){
                       ?>
                      <?php
                        ...
                        if($row->pai_menu == $row->pai_menu AND $row->funcao_id == $this->session->userdata("Funcao") ){
                             ...
                             foreach ($querysub->result() as $rowsub){
                              ...
                              <div class="col-lg-3 col-sm-6">
                                <a href="<?php echo base_url().$rowsub->link_menu;?>">
                                  <div class="card">
                                      <div class="content">
                                          <div class="row">
                                              <div class="col-xs-5">
                                                  <div class="icon-big icon-warning text-center">
                                                      <i class="<?php echo $rowsub->icone_menu;?>"></i>
                                                  </div>
                                              </div>
                                              <div class="col-xs-7">
                                                  <div class="numbers">
                                                      <p><?php echo $rowsub->nome_menu;?></p>
                                                      <?php //echo $rowsub->nome_menu;?>
                                                  </div>
                                              </div>
                                          </div>
