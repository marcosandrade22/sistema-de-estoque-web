<script type="text/javascript" language="javascript" class="init">
 $(document).ready(function() {
  ...
  <div class="content">
      ...
      <div class="content">
        <?php
          $CI =& get_instance(); if($CI->Controleacesso->acesso_funcao(35) == true){
               echo '<button class="btn btn-success" onclick="add_ajuste()"><i class="glyphicon glyphicon-plus"></i>Novo Ajuste</button>';
            }
            else{
               echo '<button class="btn btn-success" disabled><i class="glyphicon glyphicon-plus"></i>Novo Ajuste</button>';
            }
            ?>
       ...
       <table id="requisicoes" class="table-striped table-bordered" cellspacing="0" width="100%">
           ...
           <tbody>
               ...
               <tr>
                       ...
                       <td><?php echo $lista->nome_ajuste; ?></td>
                       <td><?php
                       if($lista->movimento_tipo_ajuste == 1){
                        ...
                        <td>
                          <?php echo date('d-m-Y' , strtotime($lista->data_ajuste)); ?>
                        </td>
                      <td>
                         <?php
...
function edit_produtos(id)
{
   ...
   $.ajax({
       ...
       success: function(data)
       {
           ...
           $('[name="id_ajuste"]').val(data.id_ajuste);
           $('[name="nome_ajuste"]').val(data.nome_ajuste);
           $('[name="tipo_ajuste"]').val(data.tipo_ajuste);
...
function save()
{
   ...
   if(save_method == 'add') {
       url = "<?php echo site_url('ajustes/ajuste_add')?>";
   } else {
       url = "<?php echo site_url('ajustes/ajuste_update')?>";
   }
       ...
       <div class="modal-content">
           ...
           <div class="modal-body form">
               <form action="#" id="form" class="form-horizontal">
                   <input type="hidden" value="" name="id_ajuste"/>
                   <div class="form-body">
                       <div class="form-group">
                           <label class="control-label col-md-3">Descrição do Ajuste</label>
                           <div class="col-md-9">
                               <input name="nome_ajuste" placeholder="Nome do Ajuste" class="form-control" type="text">
                               <span class="help-block"></span>
                           </div>
                       </div>

                          <div class="form-group">
                       <label class="control-label col-md-3">Tipo de Ajuste</label>
                       <div class="col-md-9">
                           <select class="form-control" name="tipo_ajuste" >
                        <?php
                           foreach($tipo_ajustes->result() as $tipo_ajustes):
                           echo ('<option value="'.$tipo_ajustes->id_tipo_ajuste.'">'.$tipo_ajustes->nome_tipo_ajuste.'</option>');
                           endforeach;
                       ?>
                       </select>
                           <span class="help-block"></span>
